  ...
  return (
    <nav className={`fixed top-0 w-full z-[9998] ${navBg} ${centerMenu ? 'py-4' : 'py-0.25'} ${hideOnModal ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'} ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'} transition-all duration-300 ease-in-out`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex menu-font text-sm sm:text-sm lg:text-base justify-center items-center space-x-1 sm:space-x-2 lg:space-x-4 flex-1">
            ...
            <div className="relative group flex items-center">
              ...
              {/* Sous-menu Presse */}
              <div className="absolute left-1/2 transform -translate-x-1/2 top-full mt-6 w-80 bg-black/90 backdrop-blur-sm shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[9999]">
                <Link href="/presse-btob" className="disco-link nav-link block px-4 py-0.5 text-[#878787] hover:text-[#FFB6C1]  uppercase text-xs">
                  PRESSE BTOB
                </Link>
                <Link href="/presse-grand-public" className="disco-link nav-link block px-4 py-0.5 text-[#878787] hover:text-[#FFB6C1]  uppercase text-xs">
                  PRESSE GRAND PUBLIC
                </Link>
                <Link href="/presse-internationale" className="disco-link nav-link block px-4 py-0.5 text-[#878787] hover:text-[#FFB6C1]  uppercase text-xs">
                  PRESSE INTERNATIONALE
                </Link>
                <Link href="/tv" className="disco-link nav-link block px-4 py-0.5 text-[#878787] hover:text-[#FFB6C1]  uppercase text-xs">
                  TV
                </Link>
              </div>
            </div>
        ...
        {/* Menu mobile overlay */}
        {isMenuOpen && (
          <div className="lg:hidden fixed top-0 left-0 w-80 bg-black/90 backdrop-blur-sm shadow-lg z-[9999] animate-slide-in-left mobile-overlay" style={{ height: 'calc(100vh - 4rem)' }}>
